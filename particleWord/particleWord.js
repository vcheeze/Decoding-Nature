// comment
var myWords = ["Peter", "is", "in", "Decoding", "Nature", "an", "NYUABUDHABI", "class"];
var myWordsIndex = 0;
...
function mousePressed() {
	...
	init(myWords[myWordsIndex]);

	myWordsIndex++;
	if (myWordsIndex == myWords.length) {
		myWordsIndex = 0;
	}
...
function readWords(word) { // word = "Luize"
	words=word;
	txt=words.split('\n');
}
    
function init(camel) { // camel = "Luize"
	readWords(camel);
