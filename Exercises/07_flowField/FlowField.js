function FlowField(r) {
  ...
  this.init = function() {
    ...
    // all flow fields point toward center of canvas
    for (var i = 0; i < this.cols; i++) {
      for (var j = 0; j < this.rows; j++) {
        var position = createVector(i*this.resolution, j*this.resolution);
        var dist = p5.Vector.sub(this.center, position);
        var theta = dist.heading();
        this.field[i][j] = createVector(cos(theta), sin(theta));
      }
    }
  };
